...
| redeemables_rollback_order_mode</br>`string` | <p>Defines the rollback mode for the order. <code>WITH_ORDER</code> is a default setting. <code>WITHOUT_ORDER</code> allows rolling the redemption back without affecting order data.</p> Available values: `WITH_ORDER`, `WITHOUT_ORDER` |
