...
| no_effect_skip_categories</br>`array` | <p>Lists category IDs. Redeemables with a given category are skipped even if the <code>redeemables_no_effect_rule</code> is set to <code>REDEEM_ANYWAY</code>.</p> |
| no_effect_redeem_anyway_categories</br>`array` | <p>Lists category IDs. Redeemables with a given category are redeemed anyway even if the <code>redeemables_no_effect_rule</code> is set to <code>SKIP</code>.</p> |
