...
const main = async () => {
      ...
      return [
        ...
        Object.fromEntries(
          Object.entries(pathEntry).map(([method, methodEntry]) => {
            ...
            dataStructure.push({ method, path, title, group });
  ...
  // .Object.keys(paths)
  // .forEach((webhookKey) => {
  //   const pathData = paths[webhookKey];
  //   const title = webhookKey.split(".").slice(2);
  //   const group = webhookKey.split(".")[1];
  //   const method = Object.keys(pathData)[0];
  //   const schemaRef = pathData[method].requestBody.content[
  //     "application/json"
  //   ].schema.$ref
  //     .split("/")
  //     .at(-1);
  ...
  openApiWebhooksContent.tags = Object.keys(dataStructureGroupedById).map(
    (key) => ({ name: capitalize(key), description: capitalize(key) })
  );
  console.log(openApiWebhooksContent.paths);
  return;
