...
const main = async() => {
    ...
    const openAPIContent = JSON.parse((await fsPromises.readFile(openApiPath)).toString())
    ...
    const parametersNames = pathsStringify.match(/"#\/components\/parameters\/.*?"/g).map(match => match.replace('"#/components/parameters/',
        '').slice(0,-1)).sort()
    const parameters = {}
    for(const parameterName of parametersNames){
        ...
        parameters[parameterName] =openAPIContent.components.parameters[parameterName]
    }

    // Removing not used schemas
    const schemas = {}
    const schemasNames = pathsStringify.match(/"#\/components\/schemas\/.*?"/g).map(match => match.replace('"#/components/schemas/',
        '').slice(0,-1)).sort()
    ...
    await fsPromises.writeFile(path.join(__dirname, '../tmp/reference/OpenAPI.json'), JSON.stringify(openAPIContent, null, 2) )
}
