...
<Note>
To add new codes to an existing campaign and set their metadata, see [Add vouchers to a campaign](/orchestrate/campaign-maintenance#add-vouchers-to-a-campaign).
</Note>
...
Manage vouchers programmatically with the Voucherify API. Each voucher is identified by a unique `code` or Voucherify-assigned `id`, and belongs to a campaign (or is a generic voucher). Use these endpoints to find, update, enable or disable, and delete vouchers, as well as to adjust gift card balances or loyalty card points.
