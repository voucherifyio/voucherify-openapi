...
## Add vouchers to an existing campaign

You can add vouchers to an existing campaign without creating a new campaign.
...
<Step title="Open vouchers">
Go to the **Vouchers** tab and click the plus icon.
</Step>

<Step title="Add vouchers">
Choose whether to add vouchers to the campaign or create a voucher with a custom code.

Define how many vouchers to generate and optionally provide a description.
</Step>
</Steps>

<Note>
You can also add vouchers programmatically with the [Add vouchers to campaign](/api-reference/campaigns/add-vouchers-to-campaign) endpoint.
</Note>

## Modify existing vouchers and gift cards

You can modify some generated voucher and gift card parameters from the Dashboard or with the Voucher API.
...
- Metadata
- Validation rules assigned directly to the voucher

Other voucher fields cannot be modified after generation.

For gift cards, you can also adjust the gift card balance from the Dashboard or with the Adjust Voucher Balance API.

<Note>
You can update vouchers programmatically with the [Update voucher](/api-reference/vouchers/update-voucher) endpoint. To update vouchers in bulk, use the Update Vouchers in Bulk endpoint.
</Note>
