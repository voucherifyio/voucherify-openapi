...
- Campaign name.
- Code generation settings; However, when you [add vouchers to a campaign](#add-vouchers-to-a-campaign), you can define new code settings.
...
Add new codes to an existing code campaign without editing the campaign itself. New vouchers inherit the campaign configuration, but you can define new code generation settings and set voucher metadata for the codes you add.
...
<Step title="Add vouchers">
Click the **+** button to add vouchers to the campaign.

You can add a single voucher with a specific code, or generate a batch of codes.
</Step>

<Step title="Set voucher metadata">
Provide values for the voucher metadata fields.

The fields shown depend on which metadata schema the campaign uses:

| Campaign setting | What you see |
|---|---|
| **Use the voucher's metadata schema** is selected, and the campaign already has metadata values | The voucher metadata fields are prepopulated with the campaign's values. You can change them for the vouchers you're adding. |
| The campaign uses the campaign metadata schema | The voucher metadata schema is shown with no values prepopulated. |

</Step>

<Step title="Save">
Confirm to add the vouchers to the campaign.
</Step>

</Steps>

Metadata values set here apply only to the vouchers you add. They don't change the campaign metadata or the metadata of codes that already exist in the campaign.

<Note>
To add many codes at once from a file, use code import instead:
...
<AccordionGroup>
  ...
  <Accordion title="Analyze campaign performance">
  ...
  | `POST` | `/v1/campaigns/{campaignId}/vouchers/{code}` | Add a voucher with a specific code to a campaign |
