...
<Update label="24 September 2026" description="v20260901_k" rss={{ title: "Workflows: react to Loyalty v2 events in your own systems" }}>

## Added

### Workflows

Workflows are now available in the Dashboard for loyalty v2. Instead of polling the API, you pick the exact event you care about and point it at the application that needs it.

You build a workflow on a canvas from **triggers** that start a run and **actions** that handle the event. Drag them in, connect them, publish, and set the workflow to **Active**. One trigger can feed several actions, and one action can accept several triggers.

This first release covers:
- **Loyalty v2 transaction** trigger: point movements, orders paid with points, tier changes, reward purchases and refunds, and benefit fulfillment, with an optional filter on transaction status.
- **Loyalty v2 activity** trigger: lifecycle events for programs, members, cards, card definitions, earning rules, rewards, benefits, and tier structures.
- **Webhook callout** action: a signed HTTP `POST` to any `http` or `https` endpoint, with up to 10 custom headers. Verify the `x-voucherify-signature` header, and inspect or retry deliveries in [Webhook sendouts](/analyze/audit-logs#webhook-sendouts).

Saving, publishing, and activating are separate steps, so you can edit a live workflow without anything changing until you publish. Runs already in progress finish on the version they started with.

### Workflows webhooks reference

The API reference now documents the Workflows payload: `source.object` is always `workflow`, `source.id` is the workflow definition ID, and `data.trigger` always holds the event that started the run. Every supported Loyalty v2 transaction and activity event has its own page with a sample payload. [Introduction to Voucherify webhooks](/api-reference/introduction-to-webhooks) now covers all three webhook types and the shared payload fields.

Go to the following pages to learn more:
- [Workflows overview](/integrations/workflows)
- [Build workflows](/integrations/build-workflows)
- [Workflow options and limits](/integrations/workflow-options-limits)
- [Workflows webhooks](/api-reference/workflows-webhooks)
...
- **Execution and run history**: Full visibility into workflow activity, with run tracking and a status breakdown of everything that passed or failed.

</Update>

<Update label="22 September 2026" description="v20260903_k" rss={{ title: "Improvements to security and bug fixes" }}>

## Improved
- **Security**: Newly created passwords have additional requirements. In addition to the existing requirements (at least 12 characters, at least one uppercase, one lowercase, and one number), they must be hard to guess and not contain email, names, keyboard patterns, or common words.

## Fixed
- **Distribution**: Fixed an edge-case issue where the distribution skipped the send-out to some customers in a segment for very large distributions.
- **Security**: Fixed an edge-case issue with SAML login that still required some users to accept the invite to the organization in order to login.

</Update>

<Update label="21 September 2026" description="v20260909_o" rss={{ title: "Refreshed UI components" }}>

## Improved
- **Refreshed UI components**: Voucherify's underlying interface engine has been refreshed. You may notice subtle visual refinements, updated component styles, and smoother layout transitions.

</Update>

<Update label="15 September 2026" description="v20260904_l" rss={{ title: "New endpoint and improvements to Loyalty v2" }}>

## Added

- **Tier structures** (API): You can now define multiple tier levels within a single tier structure request. Each structure can be configured based on points, customer segments, or both to provide more flexible loyalty program management. Use the POST `/v2/loyalties/tier-structures/{tierStructureId}/tiers/batch` endpoint to add, update, or remove tier levels in one request. On an active structure, the batch can only update tier names and metadata.

## Improved

- **Builder action buttons**: **Add tier level**, **Add earning**, and **Add spending** stay pinned to the bottom of the wizard step, so you no longer scroll back to the top when building longer lists.
- **Point expiration**: Expiration types for tiers and earning rules now follow your point wallet configuration when setting up overrides. Only matching options stay selectable and a clear error appears under the override card if the wallet setting later changes.
- **Point wallet builder**: You can now create a custom event schema directly from the activity picker when setting up points expiration or pending points, without opening advanced filtering.
- **Tier structures**:
  - You can no longer deactivate a tier structure that is assigned to an active point wallet, so live loyalty programs stay stable and members aren't disrupted by accidental changes.
  - Tiers sharing the same minimum or maximum value are now reported as one warning per value instead of one per pair, leaving more room to edit tier levels in the wizard.

</Update>


<Update label="9 September 2026" description="v20260902_o" rss={{ title: "Improvements to categories and other features" }}>

## Improved
- **Categories**:
  - Categories now appear consistently as resource labels across campaign views, with tooltips showing the category name and hierarchy value.
  - Creating a category from a campaign now opens the new wizard used in **Stacking rules**, with clearer hierarchy guidance on how priority affects campaign order when multiple redemptions apply.
- **Filters**: Quick filters on the campaigns list are deselected by default and apply only after you choose specific options. Deselecting all filters no longer hides every campaign. The full list stays visible until you actively narrow it with a filter.
- **Permissions**: Users with limited campaign-update permissions can no longer enable or disable promotion tiers directly. The action opens an approval request and takes effect only after it is approved.
- **Webhooks**: When creating a custom webhook distribution, **Send raw payload** is enabled automatically for custom payloads and locked off for default payloads, with payload settings shown more clearly in one line.

## Fixed
- **Roles**: The **Merchant** role no longer sees the **Validations** tab under **Redemptions**, and deep links to that view redirect to **Welcome** instead of loading the list and returning API errors.

</Update>

<Update label="2 September 2026" description="v20260825_l" rss={{ title: "Loyalty v2 improvements" }}>

## Improved
- **Editing loyalty v2 resources**: When editing, activating, or deactivating benefits, point wallets, earning rules, tier structures, or rewards, you will now see a list of all connected loyalty programs and entities that may be affected by your changes. This visibility change helps you understand the full impact of your edits before you save them.
- **Point expiration**: You can now view and manage expiring points on cards that are set to never expire, but have specific earning rules that override this setting. The points earned through point expiration override are correctly listed, manually adjustable, and automatically expired according to their defined rules.
- **Rewards**:
  - You can now set purchase limitations to control how many times or how often a loyalty member can redeem a reward. This includes defining specific cooldown intervals and maximum trigger limits per day, week, or month.
  - The purchase limit logic has been updated to ensure that refunded, cancelled, or rejected reward transactions are no longer counted towards total purchase limits. Unsuccessful or reversed orders are now excluded from the calculation.


## Fixed
- **Rewards**: Fixed an issue where purchasing free (zero-point cost) rewards was incorrectly blocked for members with a negative card balance. Users can now successfully claim zero-cost rewards regardless of their current point balance.

</Update>

<Update label="1 September 2026" description="v20260817_o" rss={{ title: "Quick filters for Campaigns, improved table layouts, and other UI improvements" }}>

## Added
- **Quick filters for Campaigns**: You can now use quick filters in the Campaigns table to narrow results by campaign type with an easier and consistent control. Filters update the view in real time.

## Improved
- **Combined promotions table**: Combined promotions now use a compact table with sticky names and clear columns for campaigns, categories, and tiers.
- **Horizontal scroll indicators**: Wide tables show clearer left/right cues when more columns are available off-screen.
- **Inactive input readability**: Non-editable fields keep clear input structure and text contrast so values stay readable while still looking non-interactive.
- **Project ID**: Project Settings displays the current project ID (not editable or deletable).
- **Promotion tiers table**: Promotion tiers moved from large cards to a compact table so you can scan and compare tiers more easily, with key details (including discount and category) and sticky row actions.
- **Templates table**: Campaign templates now appear in a compact table with type, description, discount details, and dedicated actions, aligned with other dashboard lists.
- **Webhook name display**: Webhook names in Project Settings use more of the popup width so longer names are easier to read, with tooltips when text still truncates.

## Fixed
- **Editing a validation subrule**: Pressing the ESC key while editing a subrule closes only the subrule modal, not the whole rule editor, so you don't lose in-progress edits.
- **Long values in filters**: Long filter values (for example campaign names) are truncated to fit the filter badge, with the full value available as a tooltip.

</Update>

<Update label="28 August 2026" description="v20260812_k hotfix" rss={{ title: "Hotfix to login" }}>

## Fixed
- **SAML login**: Fixed an issue where SAML login used in combination with projects on different clusters (e.g. dedicated and shared) caused the login process to display a "Wrong API endpoint" error. This occurred when the user logged out from a project that is on a different cluster than the one with the SAML configuration.
- **Switching between projects on different clusters**: Fixed an issue where switching between projects on different clusters caused the user to be logged out from the application. Now, the user is redirected directly to the project on the other cluster.

</Update>

<Update label="26 August 2026" description="v20260812_k" rss={{ title: "Fix to Klaviyo integration" }}>

## Fixed
- **Klaviyo integration**: Fixed an issue where distribution with Klaviyo sporadically tried to create a new customer profile in Klaviyo instead of updating an existing one due to failing to find the profile in Klaviyo, which resulted in a 409 conflict error for the creation request.

</Update>

<Update label="21 August 2026" description="v20260824_l hotfix" rss={{ title: "Hotfix to v20260824_l" }}>
...
<Update label="20 August 2026" description="v20260824_l" rss={{ title: "Dedicated loyalty resource pages, higher tier limits, and member metadata editing" }}>
...
<Update label="17 August 2026" description="v20260730_o" rss={{ title: "SKU drawers, error message library, and a cleaner voucher and campaign list" }}>
...
<Update label="10 August 2026" description="v20260807_l" rss={{ title: "Loyalty v2 now available" }}>
...
<Update label="28 July 2026" description="v20260727_l" rss={{ title: "Improvements to transfers, UI, and campaign schedules" }}>
...
<Update label="22 July 2026" description="v20260707_k" rss={{ title: "Areas API, OAuth 2.0 v2, qualification fixes" }}>
...
<Update label="15 July 2026" description="v20260713_l" rss={{ title: "Fixes to the formula builder" }}>
...
<Update label="7 July 2026" description="v20260617_k" rss={{ title: "Improved valid on specific hours" }}>
...
<Update label="6 July 2026" description="v20260622_o" rss={{ title: "Fix to Activity tabs in the UI" }}>
...
<Update label="1 July 2026" description="v20260609_k" rss={{ title: "API fixes to rollbacks and listing publications" }}>
...
<Update label="22 June 2026" description="v20260616_o" rss={{ title: "Accurate dashboard metrics and reliable voucher exports" }}>
...
<Update label="16 June 2026" description="v20260608_o" rss={{ title: "New discount formula builder" }}>
...
<Update label="9 June 2026" description="v20260602_k" rss={{ title: "Management API: removed `cockpits` property" }}>
...
<Update label="8 June 2026" description="v20260528_o" rss={{ title: "Campaign list improvements" }}>
...
<Update label="2 June 2026" description="v20260526_k hotfix" rss={{ title: "Hotfix to v20260526_k" }}>
...
<Update label="2 June 2026" description="v20260526_k" rss={{ title: "Qualification fixes" }}>
...
<Update label="28 May 2026" description="v20260519_o" rss={{ title: "Permission and discount builder fixes" }}>
...
<Update label="21 May 2026" description="v20260505_k" rss={{ title: "New endpoints for metadata schemas and improvements to other endpoints" }}>
...
<Update label="19 May 2026" description="v20260506_o" rss={{ title: "Referral updates, approval workflows, and stacking rule improvements" }}>
