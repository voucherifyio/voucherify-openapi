...
<Update label="20 May 2026" description="v20260505_k" rss={{ title: "New endpoints for metadata schemas and improvements to other endpoints" }}>
...
### Create campaign: Standalone support

Standalone campaigns can now be created through the API (POST Create campaign). This provides the same functionality previously available only through the Dashboard. The update ensures consistent experience for managing campaigns across all integration methods.

### List segments

Updated the pagination parameters for this endpoint to use `start_date` and `end_date` for easier navigation through data results.
