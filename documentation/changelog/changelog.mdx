...
These endpoints are available when your project has **Areas and Stores for campaigns** or **Areas and Stores for products** enabled (either feature satisfies the requirement).

### OAuth service API keys

You can now create **OAuth Service API keys** in **Project settings** or **Team settings** > **OAuth Service** (global, multi-project). Assign a role to each key to control which permissions its tokens can carry.
