...
- **Promotion budget**: Voucherify offers a safety net that works hand in hand with customer profile- and cart-based [validation rules](/personalize/validation-rules-reference). You can configure a campaign in a way that automatically stops when its target is satisfied, for example:
