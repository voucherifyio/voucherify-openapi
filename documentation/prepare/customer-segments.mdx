...
Passive segments update automatically when customers meet or stop meeting the defined filters. Passive segments do not generate events when a customer enters or leaves the segment. This makes them useful for validation rules, where the system only needs to check if a customer is eligible at the moment.
