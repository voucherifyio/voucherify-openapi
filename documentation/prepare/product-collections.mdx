...
<Accordion title="Use collections in validation rules">
  ...
  You can use product collections in [validation rules](/personalize/validation-rules-reference) to control when a promotion applies. For example, you can check if an order contains any item from a collection or make sure all items in the cart belong to the same collection.
