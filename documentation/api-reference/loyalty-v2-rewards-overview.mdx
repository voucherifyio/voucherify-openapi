...
Members redeem rewards through the [member reward-purchase endpoint](/api-reference/programs/purchase-reward-with-points), which spend points from the card tied to the reward's cost configuration and can later be [refunded](/api-reference/programs/refund-reward-purchase). Before building a redemption flow, use the [Examine overview](/api-reference/loyalty-v2-examine-overview) endpoint to check which rewards a member can currently afford and why any others are unavailable.
