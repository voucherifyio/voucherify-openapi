...
description: "Use webhooks to get notified about project, distribution, and Workflows events in Voucherify"
keywords: ['project', 'distribution', 'workflows', 'webhook', 'structure', 'callback', 'tracking', 'monitoring', 'notification', 'event']
---
...
Voucherify offers three types of webhooks:
...
* [Workflows webhooks](/api-reference/workflows-webhooks) – sent by a Webhook callout action after a Loyalty transaction or Loyalty activity triggers a workflow.
...
- `created_at` (`string`): Timestamp representing the date and time when the webhook send out was created. The value is in ISO 8601 format. Example: `2024-01-01T11:11:11.111Z`
- `type` (`string`): Name of the event that triggered the webhook send out. The value depends on the webhook type, for example a Project settings event name, a distribution event name, or a Loyalty v2 transaction or activity type.
- `data` (`object`): Payload of the event that triggered the webhook. The contents depend on what triggered the webhook.
- `source` (`object`): Details about the source of the webhook send out. Includes:
  - `id` (`string`): Identifier of the source that sent the webhook, such as a Project settings webhook, a distribution, or a Workflows callout step.
  - `object` (`string`): Type of the source object; `webhook` for Project settings webhooks, `distribution` for Distribution webhooks, `workflow` for Workflows webhook callouts.
  - `target_url` (`string`): URL that receives the webhook.
- `event` (`object`): Metadata of the event that triggered the send out, such as the event ID, type, timestamps, related entity ID, and request group ID. Nested properties can vary by webhook type.

The values for `type`, `data`, and `event` depend on what triggers the webhook. For Workflows webhooks, `data.trigger` is always the trigger that started the workflow. Learn more in [Project setting webhook payload](/api-reference/project-settings-webhooks#project-setting-webhook-payload), [Distribution webhook payload](/api-reference/distribution-webhooks#distribution-webhook-payload), and [Workflows webhooks](/api-reference/workflows-webhooks).
