...
* [Project-level webhooks](/reference/project-settings-webhooks) – triggered by general events across your entire project (like voucher redemption, customer updates, or order events).
* [Distribution-based webhooks](/reference/distribution-webhooks) – triggered specifically by voucher distribution events (for example, when a voucher is successfully sent or fails to send).
