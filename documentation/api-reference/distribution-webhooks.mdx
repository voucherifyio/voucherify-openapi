...
description: "Use webhooks to get notified about distribution events in Voucherify"
keywords: ['webhook', 'payload', 'configure', 'distribution', 'event', 'webhook payload']
---
...
Choose **Default** or **Custom** to determine the payload structure:
- **Default** sends the standard Voucherify webhook payload. **Send raw payload** is cleared and unavailable.
- **Custom** lets you design the contents of the `data` object. When you create a distribution and select **Custom**, **Send raw payload** is selected automatically. You can clear it if you want to send your custom payload inside the `data` object.

When you edit a webhook that uses a custom payload, Voucherify preserves the saved **Send raw payload** setting.

<Note>

  Existing webhooks may use a legacy configuration where **Default** and **Send raw payload** are both selected. When you edit such a webhook, **Send raw payload** remains selected and available so that saving the distribution does not change its payload structure. If you clear and save the setting, you cannot select it again while using **Default**.

</Note>

**Send raw payload** controls where Voucherify places your custom payload:
- When selected, the custom payload is sent at the root level.
- When cleared, the custom payload is sent inside the `data` object of the standard Voucherify webhook payload.

The **Show output** panel displays an example payload and updates dynamically based on the selected trigger context, so you see only the variables that apply to that specific trigger type.
