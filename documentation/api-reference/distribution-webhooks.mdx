...
The Raw payload checkbox lets you choose whether the webhook payload is sent at the root level or inside the data object. The Show output panel displays an example payload and updates dynamically based on the selected trigger context so you see only the variables that apply to that specific trigger type. Example values follow a simple format: strings start with `example_`, dates use `2000-01-01T00:00:00Z`, and metadata keys clearly indicate example data.
...
Here are two examples showing how the payload updates dynamically.

1. All objects included
...
```
{
  ...
  "Body": "Hello {{customer.name}}. Here's your discount code: {{voucher.code}}.",
  "OrderSummary": {
    "OrderId": "{{order.id}}",
  ...
  "DetailedRedemption": "{{redemption}}"
}
```

2. Without order object
...
```
{
  ...
  "Body": "Hi {{customer.name}}, your voucher: {{voucher.code}}.",
  "RedemptionId": "{{redemption.id}}",
  "CampaignName": "{{redemption.campaign_name}}",
  "DiscountedAmountText": "{{redemption.discount_amount}}",
  "DetailedRedemption": "{{redemption}}"
}
```

<Note>
The availability of fields (for example `customer`, `order`, `voucher`, `redemption`) depends on the trigger context.
Type `{{` in the Payload Designer to see a list of available variables.
</Note>
