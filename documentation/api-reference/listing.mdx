...
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `limit`            | A limit on the number of objects to be returned; between 1 and 100. If no limit is set, the API returns 10 items.                                                                                                                                                                                                                                                               |
...
Some of the list API methods use the `page` query parameter to display another page of results. The lowest `page` value is `1`. The highest value depends on the endpoint, for example `99` for List Vouchers, List Customers, and List Redemptions, and `100` for List Campaigns and List Publications. Check the endpoint reference for the exact range.

To scroll through large data sets, use the [Exports API](/api-reference/exports/create-export) instead of paging. For example, [List Publications](/api-reference/publications/list-publications) returns a `page_over_limit` error above page 1000.
