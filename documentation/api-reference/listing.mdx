...
keywords: ['resource', 'listing', 'data', 'response']
---
...
* [List customer redeemables](/api-reference/customers/list-customer-redeemables)
* [List referral code holders](/api-reference/referrals/referrals-code-holders)
* [List referral code holders (with campaign ID)](/api-reference/referrals/referrals-code-holders-with-campaign-id)
...
* [Management – List campaign templates](/api-reference/management/list-campaign-templates/management-list-campaign-templates)
...
To match any of several values on a single field, use the `$in` operator and pass each value as an indexed array element. Use `$not_in` to exclude a set of values.

The shortcut `category` query parameter on [List Vouchers](/api-reference/vouchers/list-vouchers) accepts a single value. To return vouchers belonging to any of several categories, use `$in` in the advanced filters instead:

```bash Filter vouchers by multiple categories
GET {api-url}/v1/vouchers?filters[category][conditions][$in][0]=pulsa&filters[category][conditions][$in][1]=pdam
```

The same pattern works for other filterable string fields, including metadata. For example, to list vouchers whose `metadata.region` is `EU` or `UK`:

```bash Filter by multiple metadata values
GET {api-url}/v1/vouchers?filters[metadata.region][conditions][$in][0]=EU&filters[metadata.region][conditions][$in][1]=UK
```

Remember to URL-encode the brackets (`[` as `%5B`, `]` as `%5D`) when your HTTP client does not do so automatically.

The full list of supported filter operators (`$is`, `$is_not`, `$in`, `$not_in`, `$less_than`, `$more_than`, `$starts_with`, `$ends_with`, `$contains`, and others) is described in the [Validation Rule object](/api-reference/validation-rules/validation-rule-object) reference.
