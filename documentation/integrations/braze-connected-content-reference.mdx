...
<Accordion title="Display generated coupon details: Snippets">
  ...
  ```liquid Display code with prefix
...
<Accordion title="Display gift card balance update: Snippets">
  ...
  Your current balances is: {{ balances.balance | divided_by: 100.0 }}.
  ```
...
<Accordion title="Display unused coupon details: Snippets">
  ...
  Code: {{voucher.code}} — Expires: {{voucher.expiration_date | date: "%d.%m.%Y" }}
  ```

  You can further down format date and time
  - 
...
If you fetch date and time, for example `expiration_date`, you can format it according to your preference as follows

`{{voucher.expiration_date | date: "%d.%m.%Y"}}`

`{{voucher.expiration_date | date: "%d.%m.%Y o %H:%M"}}`

`{{voucher.expiration_date | date: "%B %d, %Y"}}`
