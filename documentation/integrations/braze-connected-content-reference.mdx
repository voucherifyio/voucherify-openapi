...
<Warning>
    ...
    As a result: 
    - A customer cannot receive different codes within the same Braze campaign.
...
This script retrieves all unredeemed discount coupons assigned to a Braze user (lines 15-19). It loops through the customer's vouchers and displays only the ones that have not been used.
...
This script fetches an existing incentive code from the customer's custom attributes and checks its status. If the code has not been redeemed, the message reminds the customer to use it. If the code has been redeemed, the message acknowledges it. Build a customer segment based on the expiration date and target this message at that segment.
...
If points are about to expire, display the number of expiring points and the expiration date:
