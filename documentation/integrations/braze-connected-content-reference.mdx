...
<Accordion title="Display generated coupon details: Snippets">
  ...
  If you need to confirm the code prefix in the message:

  ```liquid Display code with prefix lines
  Your personal code: {{publication.voucher.code}}
  ```
...
This Connected Content script sends a custom event to Voucherify when a Braze message is delivered. The event triggers an earning rule in your loyalty campaign, which awards points to the customer. Different customer segments can receive different point amounts based on the earning rule definition.
...
```liquid Add loyalty points with a custom event lines wrap
{% assign campaign_id = {{campaign.${dispatch_id}}} %}
{% assign customer_id = {{${user_id}}} %}
{% assign source_id = campaign_id | append: customer_id %}
...
Read the documentation of [POST Track custom event](/api-reference/events/track-custom-event) endpoint to learn more about the request and response data.

## Add credits to a gift card

<Info>

  <Badge color="gray">Prerequisite: Send gift card code as a custom attribute</Badge>
...
This Connected Content script adds credits to a customer's gift card. The gift card code is retrieved from a Braze custom attribute, and the specified amount is added to the card balance.
...
```liquid Add credits to a gift card lines wrap
{% assign campaign_id = {{campaign.${dispatch_id}}} %}
{% assign customer_id = {{${user_id}}} %}
{% assign source_id = campaign_id | append: customer_id %}
...
<Accordion title="Display gift card balance update: Snippets">
  ...
  ```liquid Display added amount lines
  We've added credits to your gift card!
  ```

  To display the current balance, combine this script with the [Fetch loyalty card balance](#fetch-loyalty-card-balance) pattern, replacing the loyalty endpoint with a voucher GET call.

</Accordion>

## List unused coupons

This Connected Content script retrieves all unredeemed promo codes assigned to a Braze user. It loops through the customer's vouchers and displays only the ones that haven't been used yet.
...
```liquid List unused coupons lines wrap
{% assign campaign_id = {{campaign.${dispatch_id}}} %}
{% assign customer_id = {{${user_id}}} %}
{% assign source_id = campaign_id | append: customer_id %}
{% assign voucherify_campaign_id = "camp_XXXXXXXXXXXXXXXXXXXXXXXX" %}
...
<Accordion title="Display unused coupon details: Snippets">
  ...
  To display each unused code:

  ```liquid Unused coupon code snippet lines
  {{voucher.code}}
  ```

  To show additional details like the discount value for each voucher:

  ```liquid Display coupon with discount lines
  Code: {{voucher.code}} — Discount: {{voucher.discount.percent_off}}% off
  ```

  To display the expiration date for urgency:

  ```liquid Display coupon expiration date lines
...
This Connected Content script validates which promotion tiers a customer qualifies for based on audience rules. It returns all eligible promotions and displays their names in the message.

Use this snippet to show personalized promotions in the following scenarios:
- Personalized deal feeds showing only promotions a customer qualifies for
- Cart abandonment emails highlighting applicable automatic discounts
- Weekly promotional digests tailored to each customer's segment
- In-app message banners surfacing the best available offer

```liquid List unused promotion tiers lines wrap
{% assign campaign_id = {{campaign.${dispatch_id}}} %}
{% assign customer_id = {{${user_id}}} %}
{% assign source_id = campaign_id | append: customer_id %}
...
<Accordion title="Display promotion tier details: Snippets">

  To display each eligible promotion name:

  ```liquid Promotion name snippet lines
  {{promotion.name}}
  ```

  To display the promotion banner (if configured):

  ```liquid Display promotion banner lines
  {{promotion.banner}}
  ```

  To show a fallback message when no promotions are available:

  ```liquid Display no promotions fallback lines
  {% if promotion_list.valid != true %}
...
<Accordion title="Display the code: Snippets">
  ...
  Place this snippet where you want to display the promo code in your message:

  ```liquid Display promo code lines
...
<Info>
  ...
  <Badge color="gray">Prerequisite: Store incentive code as a custom attribute</Badge>

  For this script to work, you need to first assign a promo code, gift card, or referral code to a Braze user and save the code and expiration date as custom attributes.
...
<Accordion title="Display resent code details: Snippets">
  ...
  ```liquid Incentive code snippet lines
  {{incentive.code}}
  ```

  To show the expiration date to create urgency:

  ```liquid Display expiration date lines
  ...
  ```liquid Conditional redemption status lines
  {% if incentive.redemption.redeemed_quantity == 0 %}
