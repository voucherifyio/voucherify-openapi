...
Iterable can then react to those events and send customer communication, such as emails.

### Example use case

In this example:
- a customer completes an action, such as a successful referral,
- Voucherify publishes a reward, for example a discount code,
- the customer should receive an email sent from Iterable.

Voucherify is responsible for detecting the event and publishing the reward.  
Iterable is used only to send the email.

### Flow

1. Voucherify publishes a reward.
2. Publishing the reward triggers a Voucherify distribution.
3. The distribution sends a custom webhook request to Iterable.
4. Iterable sends a triggered email to the customer.
...
### Step 2: Generate an Iterable API key

To allow Voucherify to send requests to Iterable:

1. In Iterable, go to **Integrations**.
...
### Step 3: Configure a Voucherify distribution trigger
...
### Step 4: Configure a custom webhook in the distribution

In the distribution action, configure a **custom webhook**.

#### Target URL

Use the Iterable endpoint that sends a triggered email, for example:
...
- `campaignId` – identifies the triggered email campaign
- `dataFields` – values used inside the email template

Values from Voucherify, such as the published discount code, are passed in `dataFields`.

---

### Step 5: Align customer identifiers
...
This mapping allows Voucherify to identify the correct recipient in Iterable.
