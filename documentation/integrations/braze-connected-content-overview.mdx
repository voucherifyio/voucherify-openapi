...
Braze Connected Content allows you to fetch data from the Voucherify API at message send time and display it directly in your Braze messages. You can publish coupons, invite new referrers, retrieve loyalty card balances, add gift card credits, and more.
...
Connected Content calls Voucherify API. The examples used in documentation call the default cluster. Change the endpoint if your project is in another region or uses a dedicated cluster.
...
Without proper optimization, each Connected Content message triggers at least two Voucherify API calls, which can drain your hourly and monthly limits. Before going to production, configure:
