...
If a Voucherify customer does not exist in Braze, the distribution creates a new Braze user profile. The Voucherify customer `source_id` is used as the Braze `user_id`.
...
<AccordionGroup>
    ...
    Before using Braze custom attributes in Voucherify distributions for the "Customer entered/left segment" trigger, you need to add your Braze users to Voucherify.
