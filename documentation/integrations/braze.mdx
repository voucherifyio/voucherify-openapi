...
**The integration with Braze consumes** [API limits](https://docs.voucherify.io/guides/limits#api-limits) **for Connected Content and** [webhook limits](https://docs.voucherify.io/guides/limits#webhooks) **for sending codes to Braze with custom attributes, as well as synchronizing the audience in Braze.**
...
Note, that campaigns may use Connected Content script to publish a unique coupon code and send it in the Braze message. More information about it in this [article](https://support.voucherify.io/article/573-braze-connected-content).
...
| **audience_id** (required) | The `audience_id` can have any value from Braze Canvas. All customers in a segment will share the same audience ID stored in Voucherify customer metadata, for example:<br /><br />```json<br />brazeAudiences: [<br />  "brazeAudienceID"<br />]<br />``` |
...
* user_id (required): email_address
  * Braze user_id: The email maps directly to Voucherify's email field
* user_id (required): first_name
  * Braze user_id: The first name and last name will concatenate in Voucherify into the customer’s name field
* user_id (required): last_name
  * Braze user_id: The first name and last name will concatenate in Voucherify into the customer’s name field
* user_id (required): country
  * Braze user_id: This maps directly to the customer's address in Voucherify (the country address parameter)
* user_id (required): city
  * Braze user_id: This maps directly to the customer's address in Voucherify (the city address parameter)
* user_id (required): phone_number
  * Braze user_id: This maps directly to the customer's address in Voucherify (the phone parameter)
...
6.  In the Compose section, add the following URL in Webhook URL.
    ...
    `https://api.voucherify.io/v1/events`
    
7.  In the Request Body, form the proper payload for the [track event](https://docs.voucherify.io/api-reference/events/track-custom-event) endpoint.
