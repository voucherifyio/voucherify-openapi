...
This integration enables data transfer from Voucherify to CleverTap, allowing you to effectively trigger action-based messaging and deliver codes to customers. Here are some scenarios where this functionality can be used:
...
Thanks to this integration, you can inform your customers about promotions and deliver personalized coupons seamlessly. Combining CleverTap campaigns with Voucherify allows you to respond to various customer actions or occasions by automatically distributing unique coupons and gift cards.

Here is an example scenario:

1.  **Re-engage inactive customers:** Identify customers who have not visited your store for a while and incentivize them with a discount coupon.
2.  **Exclusive redemption:** Ensure that only the recipient of the voucher can redeem it, maintaining exclusivity and preventing misuse.
3.  **Personalized delivery:** Send tailored coupons to customers via email, creating a more engaging and targeted experience.

## Setting up the integration

### Voucherify

In Voucherify, you will need to generate API integration keys for CleverTap.
...
In this example, a [unique coupon campaign](https://support.voucherify.io/article/46-bulk-unique-codes-campaign) called **Customer Reactivation”** is implemented. This campaign offers a 20% discount on the entire cart, with a maximum discount value of $100.
...
You will need the  **campaign ID** for later steps. You can copy it from the campaign tile in the campaign list or you can call [the GET campaign endpoint](https://docs.voucherify.io/api-reference/campaigns/get-campaign).
...
The process of assigning a [referral code](https://support.voucherify.io/article/48-referral-program-basics) or [loyalty card](https://support.voucherify.io/category/389-loyalty-campaigns) is similar to the one described above. All you need to do is change the campaign name parameter to the referral or loyalty campaign you created.
