...
Connected Content calls don't cache by default and will make two API requests per each call. This can drain your hourly and monthly limits. The caching mechanism limits that to one API call per one request.
