...
Use the following steps to display data fetched from Voucherify in Braze messages.
...
<Steps>
   ...
   ```
      ...
      <Note>
         ...
         The script has additional data provided to make the script optimized.
   ...
   - Add publications address `/v1/publications` and the API method (lines 8-9).
   - Add your API credentials for authentication (lines 11-12).

   ```liquid Connected content publication script lines highlight={4,8-9,11-12}
      {% assign braze_campaign_id = {{campaign.${api_id}}} %}
      {% assign customer_id = {{${user_id}}} %}
      {% assign source_id = braze_campaign_id | append: customer_id %}
      {% assign voucherify_campaign_id = "camp_XXXXXXXXXXXXXXXXXXXXXXXX" %}
