...
<Note>
   ...
   The connected content script example has additional data provided to make the script optimized.

   In this tutorial, only essential Voucherify data is described.

   Read the [Optimize your Connected Content scripts](/integrations/braze-connected-content-optimization) for more details.
...
<Steps>
   ...
   Responses from the Voucherify API is stored by connected content under the value of `:save` parameter. For example, `:save publication` in line 18.
   ...
   <Tip>
      ...
      You can fetch any data returned from Voucherify API.

      This tutorial uses the [POST Create publication](/api-reference/publications/create-publication) endpoint, so you can fetch other details.

      For example, if you distribute gift cards, you can also display gift card value by using the following snippet:

      ```liquid
      {{publication.voucher.gift.amount}}
      ```
      
      You can fetch different data in one message, for example:
      
      ```liquid
      
      Hi,
      
      Your gift card is ready: {{publication.voucher.code}}.
      
      You have {{publication.voucher.gift.amount}} to spend.
      ```
...
Your message is ready to fetch Voucherify data and use it in Braze campaigns.

<Tip>
   
   It's better not to entirely depend on the **Preview mode** in Braze. Send several test messages to confirm that everything works as intended.
