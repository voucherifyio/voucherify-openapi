...
The script is ready. Send test messages with **Test Send** before launching. Braze preview mode does not display Connected Content calls with a `retry` attribute.

In Voucherify, go to **Audit logs** to verify that Braze called the API successfully.
