...
<Steps>
   ...
   ```liquid Connected content publication script
   ...
   Add a Voucherify `campaign_id` copied from the campaign details view in the Voucherify dashboard.

   ```liquid Connected content publication script focus={4}
      {% assign braze_campaign_id = {{campaign.${api_id}}} %}
      {% assign customer_id = {{${user_id}}} %}
      {% assign source_id = braze_campaign_id | append: customer_id %}
      {% assign voucherify_campaign_id = "camp_XXXXXXXXXXXXXXXXXXXXXXXX" %}
