---
title: "Fetch data with Braze Connected Content"
...
The `source_id` parameter in the script body ensures each customer receives only one unique code per Braze campaign. Even if Braze sends duplicate requests, each user gets the same code that was published in the first message.
...
The following steps assume you have a Braze campaign or Canvas in which you want to use the Connected Content script. 
...
Connected Content stores the Voucherify API response under the `:save` parameter value. For example:
