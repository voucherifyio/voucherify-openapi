...
<AccordionGroup>
    ...
    <Note>
      Voucher expiration does not emit a dedicated webhook event. There is no `voucher.expired` event. The only expiration-related event is [`voucher.loyalty_card.points_expired`](/api-reference/voucher/loyalty-card-points-expired), which fires when loyalty points expire, not when a voucher itself reaches its `expiration_date`.

      To detect expired vouchers, poll the [List vouchers](/api-reference/vouchers/list-vouchers) endpoint and filter on `expiration_date`, or use the [Exports API](/api-reference/exports/create-export) to schedule voucher exports.
    </Note>
