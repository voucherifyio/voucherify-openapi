...
<Note>
  ...
  Read the [Validation rules](https://support.voucherify.io/article/529-validation-rules-campaign-limits) article to learn how each rule works.
...
<AccordionGroup>
    ...
    <Accordion title="Customer segments">
        ...
        Use [customer segments](/prepare/customer-segments) in **Audience** rules to target or exclude specific customer groups. Segments can be based on customer attributes, behavior, or metadata.

        For example, limit a promotion to customers in a "VIP" segment or exclude customers who have already used a similar offer.

    </Accordion>

    <Accordion title="Product collections">

        Use [product collections](/prepare/product-collections) in **Products** rules to define which items must be in the cart for a rule to pass.

        For example, require at least one item from a "Summer Sale" collection or exclude items from a "Clearance" collection.

    </Accordion>

    <Accordion title="Metadata">

        Use [metadata](/prepare/metadata) to create rules based on custom attributes attached to customers, orders, products, or redemptions.

        For example, check if a customer has `newsletter_subscriber: true` or if an order contains items with specific metadata values.

    </Accordion>

    <Accordion title="Earning rules">

        Attach validation rules to [loyalty earning rules](/build/earning-rules) to control when customers can earn points.

        For example, award points only for orders above \$50 or only to customers in a specific segment.

    </Accordion>

    <Accordion title="Discount campaigns">

        Validation rules are a key part of [discount coupon campaigns](/build/create-discount-coupons). Use them to limit who can redeem coupons and under what conditions.

    </Accordion>

    <Accordion title="Fraud prevention">

        Validation rules help prevent [fraud and abuse](/guides/fraud-prevention-mechanisms) by limiting redemptions per customer, per day, or per campaign.

        Use **Budget constraints** to automatically stop campaigns when limits are reached.

    </Accordion>

    <Accordion title="Developer setup: Validation Rules API">

        Developers can manage validation rules programmatically using the [Validation Rules API](/api-reference/validation-rules/validation-rule-object).

        Use the API to create, update, assign, and delete validation rules across campaigns.
