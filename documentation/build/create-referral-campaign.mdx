...
- **Points on loyalty card** – adds loyalty points to the referee's loyalty card in an existing loyalty campaign.
...
<Info>
  ...
  The **Validation rule** step is available only for **Credits on gift card** and **Points on loyalty card**. For **Discount coupon**, add **validation rule** in the **Conditions** step in the discount builder.
...
For single-sided referral campaigns, it's recommended to set the **Redemptions per customer in a campaign** rule to **less than or equal to 1**. This way, each referee can use a referral code from this campaign only once.
