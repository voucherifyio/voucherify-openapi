...
When setting up your incentive workflow in the Campaign Manager, you must decide between a single-sided or double-sided structure. This determines who receives a reward when a successful referral occurs. 
...
![Single-sided](/images/docs/referral-overview-01.png "Single-sided")        
...
![Double-sided](/images/docs/referral-overview-02.png "Double-sided")

## Conversion events

A conversion event is the specific action that triggers a successful referral in Voucherify.  Each campaign can only have one primary conversion event.
...
![Code-redemption](/images/docs/referral-code-redemption.jpg "Code-redemption")

### Custom events

Custom events are actions tracked in your own application and sent to Voucherify via the API. 

* Voucherify counts a successful referral only after receiving the API call for that specific event.
* Even if a referee redeems a code for a discount earlier, the referrer is not rewarded until the custom event occurs.
* To pass these events, you must invoke the `track custom event` endpoint, ensuring the payload includes the referee's details and the referral code.

![Custom-event](/images/docs/referral-custom-event.png "Custom-event")

## Referee rewards

In double-sided programs, you must define the incentive for the new customer. These rewards are configured under the Referee settings in the Campaign Manager.

### Reward types

You can choose from several reward categories depending on your business model:

* Discount Coupons: Applies an immediate discount (e.g., 20% off, free shipping, or a fixed amount) to the referee's order.
...
<Note> 
If a referee does not already have a loyalty or gift card, Voucherify will automatically publish one to their profile before adding the points or credits. 
</Note>

### Validation rules for referees
To prevent fraud and ensure high-quality leads, you can add Validation Rules to referee rewards.

* Common Rule: "Redemptions per customer in a campaign is 1."
* This ensures that each new customer can only be "referred" once, maintaining the integrity of your campaign budget.
