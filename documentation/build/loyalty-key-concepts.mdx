...
You can define global limits (balance-based, time-based) or per-transaction limits.
...
You can define global limits (spending-based, time-based) or per-transaction limits.
...
Examples include discount coupons, gift cards, and physical rewards (products).
...
3. The earning rule applies effects, for example adding points or issuing benefits.
4. Points are stored and managed in a point wallet.
5. The loyalty member can qualify for tiers by earning points or use them to redeem rewards or pay for orders.