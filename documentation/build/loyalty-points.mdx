...
<Info>
    ...
    You can also add or subtract points programmatically with the [Adjust Voucher Balance](/api-reference/vouchers/adjust-voucher-balance) endpoint. This endpoint accepts only `amount`, `source_id`, and `reason`. It does not accept a per-request expiration date. Points added through the API inherit the expiration configured on the loyalty campaign or the matching earning rule. To change how added points expire, update the [loyalty point expiration rules](#loyalty-point-expiration-rules) on the campaign instead.
