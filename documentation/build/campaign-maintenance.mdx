...
Go to **Campaign hub** > **Campaigns** and select the campaign. Click the edit button in the upper right right corner to open the campaign builder.
...
|---------|-------------------------|
| Metadata | If in **Metadata(optional)** in the campaign builder, the **Use the voucher's metadata schema** is selected, it affects all codes|
...
You can't change the following settings once campaign has been created:
- Campaign name.
...
| Stop the campaign permanently and block reuse its name and its codes | **Move to bin** |
...
Disabled campaigns stop working. The disabled campaigns and their codes aren't also returned in the qualification (checking eligibility).
...
Exporting large amount of data can take some time.
