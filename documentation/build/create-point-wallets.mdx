...
Point deduction is triggered by a refund. Rolling back a redemption does not deduct points earned from the related order. To remove such points manually, use the [Adjust card points](/api-reference/programs/adjust-card-points) endpoint with a negative `points` value.
