...
*   The bundle configuration dialog includes three steps:
    ...
    *   Discount: Set up the maximum number of eligible bundles, bundle scaling behavior, and other discount details, including [dynamic formula](https://support.voucherify.io/article/568-how-to-create-dynamic-discount-value).
...
Bundle scaling determines whether the discount or limit is applied once or multiplied by the number of eligible bundles in the cart.

Use this setting to control how bundle discounts behave when multiple bundles are detected in a single order.

<AccordionGroup>
  <Accordion title="Multiply discount by bundle count (default)">
    The discount scales automatically based on the number of eligible bundles in the cart.

    This option multiplies:

    - discount values, such as amount and percentage discounts
    - limits, such as the maximum number of discounted units

    **Example:**

    \$5 discount per bundle

    2 eligible bundles → \$10 total discount
  </Accordion>

  <Accordion title="Use custom formula">
    Choose this option if your formula already includes bundle quantity, for example `order.bundle.quantity`.

    This prevents double multiplication and gives you full control over how the discount or limit is calculated.
  </Accordion>
</AccordionGroup>

<Note>

Bundle scaling does **not** change the behavior of the following bundle discount types:

- **New price: Apply to whole cart**
- **New price: Apply the discount to item subtotal**

For these types, the discount value is not multiplied by the number of bundles.
...
To create a new rule, click the plus button to open the [validation rule builder](/optimize/create-validation-rules) in the Campaign Manager. You can also create validation rules in the [Validation Rules](https://support.voucherify.io/article/16-dashboard-sections#valrules) section in the dashboard for future use.
