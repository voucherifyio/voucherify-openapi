...
<Note>
  ...
  Read the [Validation rules](https://support.voucherify.io/article/529-validation-rules-campaign-limits) article to read how each rule works.
...
- **Validation rules** in in the **Campaign hub**: Create reusable rules for future use across campaigns.
...
<Steps>
  ...
  <Step title="Add rules">
    ...
    **Add rules** to choose conditions from available categories:
    ...
    Some rules have also subrules that can set additional conditions.
    ...
    **Edit rule** by clicking the three-dot menu.
    ...
    Depending on the **Context** defined in step one or the dashboard context (campaign builder, earning rule, distribution, and so on), the number and types of rules will vary.
