...
<Callout icon="notebook" color="#2196F3" iconType="regular">
  In addition to the keys, you will need to choose suitable [API endpoints](/guides/introduction-1).
</Callout>
...
The other authentication mechanism uses publishable keys. They are meant to be used by the public (web and mobile) clients to access a subset of the API (labeled with `(client-side)` in the [API reference](/api-reference)).
...
* [Generate OAuth token](/api-reference/generate-oauth-token)
* [Introspect OAuth token](/api-reference/introspect-oauth-token)
* [Revoke OAuth token](/api-reference/revoke-oauth-token)
