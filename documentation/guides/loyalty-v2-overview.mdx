...
- **Program limits**: Up to 10 wallets, 100 rewards, 100 earning rules, one tier structure (up to 100 levels), and 100,000 members per program. See [Loyalty program overview](/build/loyalty-overview#program-limits).
...
- **No native point transfer in v2**: The `/v2/loyalties` API does not expose a point-transfer endpoint. Point transfer between cards is only available through the legacy [POST Transfer Loyalty Points](/api-reference/loyalties/transfer-loyalty-points) endpoint on `/v1/loyalties`, which is a legacy-loyalty feature. Plan flows that need to move balances between members with this in mind. Partial refund handling for loyalty v2 is on the roadmap; see [Loyalty program overview](/build/loyalty-overview#new-loyalty%3A-roadmap).
...
<Card title="API reference" icon="square-terminal" href="/api-reference/loyalty-v2-api-overview">
  Open the Loyalty v2 API map and endpoint reference for request and response schemas.
</Card>

<Card title="Loyalty v2 workshop" icon="hammer" href="https://github.com/voucherifyio/loyalty-workshop">
