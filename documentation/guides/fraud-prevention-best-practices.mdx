...
Voucherify provides tools for preventing promotion fraud.

Identity fraud prevention (such as fake accounts or account takeovers) must be implemented on your side using external systems.
...
- **Redemption limits**: restrict how often incentives can be used  
...
- **Redemptions per customer per period**: prevents repeated use within short time frames  
...
You can configure webhooks in [Project settings webhooks](/api-reference/project-settings-webhooks).  
For an overview, see [Introduction to webhooks](/api-reference/introduction-to-webhooks).
...
Email and identity verification for detecting disposable or risky addresses.

Learn more: [atdata.com](https://atdata.com)
</Accordion>
...
Fraud detection using data enrichment and behavioral analysis.
