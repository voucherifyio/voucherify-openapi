...
keywords: ['API', 'overview', 'optimization', 'version', 'endpoint', 'region', 'region', 'client-side', 'performance', 'p95', 'response times', 'rate limiting', ]
---
...
If your company has a dedicated region, use its unique URL.

Each project is bound to one region. If you call a different region than the one your project is bound to, the API returns a `403` error with the `wrong_api_endpoint` key. The `details` field tells you which base URL to use:

```json
{
  "code": 403,
  "message": "Wrong API endpoint",
  "details": "Given project is bound to different API endpoint, you should use: https://us1.api.voucherify.io",
  "key": "wrong_api_endpoint"
}
```

To fix this error, replace the base URL in your requests with the URL returned in `details` and keep the rest of the request unchanged.
