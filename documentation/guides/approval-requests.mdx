...
- **Created campaigns and updates must be approved**  
  Campaign updates also require approval before they are applied.  
  When a campaign is modified by a restricted user, the **current campaign state remains unchanged** until the request is approved.  

  After the approver approves the request, the changes are applied and the **`campaign updated` event** is generated.

These limitations define which campaign actions require approval before changes are applied.
...
<Steps>
  ...
  <Step title="Add approvers">
    In the final step of the wizard, click <strong>Add approvers</strong> and select one or more users who can approve the request (for example, Admin users).
  </Step>
