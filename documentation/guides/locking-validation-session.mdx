...
description: "Temporarily lock the voucher's usage until redemption is successful."
keywords: ['session', 'mechanism', 'transaction', 'lock', 'redemption', 'validation']
---
...
### Update the cart without duplicating the lock

When a customer adds another item to the cart and you need to re-validate the same voucher, reuse the same `session.key` in the follow-up validation request. Voucherify overrides the existing session values instead of creating a new lock, so the code's redemption quota is only reduced once.

```json Follow-up validation request
{
  "redeemables": [
    { "object": "voucher", "id": "wTzxDoOk" }
  ],
  "order": {
    "amount": 250000
  },
...
If you omit `session.key` and send `{ "type": "LOCK" }` again, Voucherify treats it as a new session and locks another usage against the code's redemption limit, which can trigger `quantity_exceeded`.

### Release a session before the TTL expires

When a customer abandons the cart, changes the applied code, or moves the discount to a different item, call [Release Validation Session](/api-reference/vouchers/release-validation-session) with the current `session.key` to free the locked usage immediately. A subsequent validation returns a new `session.key` for the new context.

### Always include the session key in redemption

If you established a session during validation, the redemption request must include the same `session.type` and `session.key`. Without them, the redemption is not matched to the locked usage and the lock stays in place until the TTL expires.
