...
<AccordionGroup>
  <Accordion title="Customer segment">
    Segments group customers based on standard attributes (such as email or postal code) and custom attributes added as metadata.
  </Accordion>

  <Accordion title="Custom event">
    External actions performed by customers that are tracked and sent to Voucherify using the API.
  </Accordion>
