...
Use the [GET membership](/api-reference/programs/get-membership) endpoint (`GET /v2/loyalties/programs/{programId}/memberships/{memberId}`) to create a member profile that will show:
...
- `member` — the member's profile in the program (lines 2–11).
- `program` — a summary of the loyalty program the member belongs to (lines 12–18). This is useful when your integration handles several programs and needs to disambiguate which program the response refers to.
- `cards` — the list of loyalty cards issued to the member (starting at line 19), used to render points and tier progress.

Use the same response for the `#### Member`, `#### Points`, and `#### Tiers` subsections below.

```json Get membership response wrap lines expandable
{
    "member": {
        "id": "lmbr_128f962dbc8c4ba5dc",
        "customer_id": "cust_6onRrs4kCnvSVrTs5YtDPFBG",
...
The `member` object (lines 2–11 of the response above) holds the profile data you use to identify the member in your system and reflect their standing in the program:
- `id` is the loyalty member ID assigned by Voucherify (line 3). Store it alongside your own customer record and pass it back on subsequent calls to this endpoint.
...
- `lifetime_bucket` stores historical details regarding `points` and `pending_points` (lines 28–48). For example, you can use the data to show how many points were `earned`, `spent`, `expired`, and so on.
...
If your program runs loyalty tiers, you can also use the Get membership endpoint to return the details about the current tier progress for a given loyalty card. Use the data from the `tier_progress` object on the relevant card in the `cards` array (lines 115–151 of the response above, on the third card `AnnualTime-7M7ShPGfme`) to build a member profile that will display the active tier, and also risks and opportunities:
