...
It now contains the member lifecycle: Preparation → Acquisition → Discovery → Activity →
Retention → Fulfillment → Cancellation. The "Going to production" and "Troubleshooting"
sections moved to `loyalty-v2-take-to-production.mdx`. Content was NOT rewritten — only
outright errors (typos, broken links, invalid JSON, mislabeled code blocks) were fixed.
Inline `REVIEWER NOTE` comments flag things that need your attention.

Items flagged for your attention (not auto-changed):
  - The `SPECIFIC` trigger mode for Examine earnings is listed as an option but has no
    dedicated subsection (only `ALL` is expanded). Consider adding one.
  - Every request body is bare JSON. No full HTTP request (method + URL + `X-App-Id` /
    `X-App-Token` headers) is shown anywhere in the guide.
  - No authentication / rate-limit / idempotency / webhook guidance.
  - No SDK cross-links (dotnet/java/js submodules exist in this repo).
  - Terminology drift: "point wallet" vs "card definition", "benefits" vs "rewards",
    "physical rewards" vs "material rewards" vs `MATERIAL`. Define once and stay consistent.
  - Sentence length: many sentences in Examine earnings / Examine rewards / Pay with points
    exceed 25 words; the style guide asks for 5–15.
  - Heading case / parallelism: verb-phrase subheadings ("Examine earnings", "Pay with
    points", "Purchase reward") mixed with noun-phrase H2s ("Fulfillment", "Retention").
  - `highlight={…}` line-number pointers in prose are brittle for LLM retrieval. Prefer
    named-field references (e.g. "the `unavailability_reasons[].reason` field").
  - Broken internal links detected by `mint broken-links` (present in the original,
    not introduced by the split — resolve to real API-reference paths):
      * `/api-reference/programs/create-member-in-batch`
      * `/api-reference/examine/examine-rewards`
      * `/api-reference/programs/pay-for-an-order-with-points`
*/}
...
<Steps>
    ...
    <Step title="Create loyalty v2 program">
        ...
        <Info>
            ...
            {/* REVIEWER NOTE (mintlify): `/build/` is a directory-level link, not a specific page. Point this at the exact landing page for the Loyalty v2 user guides. */}
...
- The loyalty card(s) generated for the member with a unique loyalty card ID (`lcrd_...`) and the point wallet (card definition) ID. If your program uses multiple point wallets, the member will automatically receive a separate card for each card definition (lines 16, 17, 54, and 55).
...
{/* REVIEWER NOTE (mintlify): Examine earnings / Examine rewards rely heavily on `highlight={…}` line-number pointers in the prose ("lines 9–40", "lines 56–128"). These break when the LLM retrieves prose without the full code block. Prefer named-field references — e.g. `earning_rules` array, `unavailability_reasons[].reason` — with line numbers as a secondary cue. */}
...
{/* REVIEWER NOTE (mintlify): `SPECIFIC` is listed here but has no dedicated subsection below (only `ALL` is expanded). Add a "SPECIFIC trigger" subsection, or remove it from this list. */}
...
{/* REVIEWER NOTE (mintlify): leftover TODO placeholder — decide whether to finish this tip about `order.source_id` from a third-party system, or delete it before merging.

<Tip>

    <Badge color="green">Order management system</Badge>

    Sth sth sth how to get order.source_id from a third-party system
