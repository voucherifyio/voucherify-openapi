...
Metadata (custom attributes) extends the customization capabilities of Voucherify. Metadata is custom key–value data you can add to objects (customers, orders, products, campaigns, and more) to control targeting, optimize and personalize incentives, and improve tracking and reporting. 
...
Each Voucherify resource can contain **up to 100 definitions**.
...
- **Image URL**: Represents image's URL.
...
<Steps>
  ...
  <Step title="Define metadata details">
    ...
    <Warning>
      ...
      <Badge color="yellow">Required metadata: Definitions and requests</Badge>
  ...
  <Step title="Select the metadata type">
    ...
    The selected type controls which <Tooltip headline="Validation rule" tip="Validation rules help you limit the application of incentives for better targeting." cta="Read Validation rule guide" href="https://support.voucherify.io/article/529-validation-rules-campaign-limits">validation rules</Tooltip> you can define. Some metadata types can't be used in validation rules, for example **geopoint**.
...
<Tip>
  ...
  There, you can edit property name, define if it's **optional** or **required** and if it contains a **single** value or **multpile** values of a given **Type**.
...
You can copy metadata schema definitions from another project to add it to your existing definition.
...
<AccordionGroup>
  ...
  <Accordion title="Metadata in customers">
    ...
    Metadata added to the **Customer** schema extends basic attributes assigned to each customer in Voucherify. In this way, Voucherify can reflect your CRM data structure. Each metadata attribute can later serve as a redemption limit or filter in [segmenting](/prepare/customer-segments) your audience.
  ...
  <Accordion title="Metadata in validation rules">
    ...
    You can use metadata schemas to build fine-grained [validation rules](https://support.voucherify.io/article/529-validation-rules-campaign-limits) that determine your desired redemption circumstances. The validation rule builder can create limits on customer, product, order, and redemption metadata.
