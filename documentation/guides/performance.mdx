...
keywords: ['response', 'rate', 'SLA', 'tenant']
---

Fast API response times are critical, especially in the sales funnel. Voucherify is optimized for speed. Here are average response rates from recent tests on multi-tenant clusters.
...
Every time you send a request to Voucherify, Voucherify reacts synchronously and asynchronously. Voucherify uses this approach to return results as fast as possible, making the customer experience seamless.

When requesting a redemption, Voucherify synchronously validates the voucher, returns the result, and you can expose the result to the customer. However, this doesn't mean Voucherify has finished processing the request.
