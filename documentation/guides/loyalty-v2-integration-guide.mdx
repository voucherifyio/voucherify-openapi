...
<Info>
    ...
    <Badge color="gray">Prerequisite: Settings of pay with points in the oint wallet</Badge>

    The point-to-currency exchange ratio is defined in the point wallet that governs the behavior of the loyalty card.

    You can create a simple exchange ratio (for example, \$0.01 equals 5 points) or build a dynamic formula to calculate the exchange ratio based on defined conditions (for example, if the member is in the VIP segment, \$0.01 equals 1 point; for other members, it equals 5 points).
...
- Two required fields:
    - The `card_id` string of the loyalty card (`lcrd_...`) where the points to be spent on the order are stored.
    - The `order` object that contains the `id` or `source_id` of the order to be paid.
- Two optional fields:
    - The `payment_limit` object that defines a limit on the transaction by providing a `type`:
        - `CARD_BALANCE` default that allows the automatic payment up to the maximum of the total balance of the loyalty card.
        - `POINTS_LIMIT` that uses the `points_limit` object to define how many points will be spent in the transaction.
        - `AMOUNT_LIMIT` that uses the `amount_limit` object to define how much of the order amount can be paid for with points.
    - The `mode` string that defines if the request is used to simulate the transaction or to actually pay for ther order. Pass the following values:
        - `DRY_RUN` to perform a simulation of the transaction. No points are spent from the member loyalty card.
