...
Use the [POST Create member](/api-reference/programs/create-member) (`/api/v2/loyalties/programs/{programId}/members`) or [POST Create member in batch](/api-reference/programs/create-member-in-batch) (`/api/v2/loyalties/programs/{programId}/members/batch`; asynchronous action) with the Voucherify customer ID to add the customer as a member to a loyalty v2 program.
...
<Tip>
    ...
    - Customers with a defined metadata `"cat_owner": true`.
...
Use the following endpoints and data to create a member profile in your system and which helps retaining your member base and keeps it engaged.

### GET Member

<Info>

    Read the [GET Get member](/api-reference/programs/get-member) endpoint reference documentation to learn more about the exact request and response data.
    
</Info>

Use the GET Get member endpoint (`/v2/loyalties/programs/{programId}/members/{memberId}`) to create a member profile that will show:
...
With the Get member endpoint you can create a member profile that lists points available to a member. Each of the member's loyalty card is returned in the `cards` array (starting in line 10). The details regarding points are stored in the following objects:
...
<Info>
    ...
    Read the [GET Get list transactions](/api-reference/programs/list-card-transactions) endpoint reference documentation to learn more about the exact request and response data.
    
</Info>

Use the GET List card transactions (`/v2/loyalties/programs/{programId}/members/{memberId}/cards/{cardId}/transactions`) endpoint to extend the member profile in your system with details about all point movements on a member's card. If your loyalty program uses several point wallets, create a separate transaction list for each card that's governed by a given wallet.
...
- `PENDING_POINTS_ADDED`: Pending points that were added to the loyalty card under the point wallet settings.
...
- `POINTS_EARNED`: Points earned and instantly added to the loyalty card (point earned example, line 21–24).

<CodeGroup>

```json Example: List card transactions response wrap expandable lines highlight={11-24,38-47,90-108}
{
    "data": [
        {
...
```json Point earned response wrap lines expandable higlight={21-24}
{
    "data": [
        {
...
<Info>
    ...
    Read the [GET Get list reward purchases](/api-reference/api-reference/programs/list-reward-purchases) endpoint reference documentation to learn more about the exact request and response data.
    
</Info>

Use the GET List reward purchases (`v2/loyalties/programs/{programId}/members/{memberId}/rewards/purchases`) endpoint to show your members the rewards they have purchased. This endpoint lists purchases for all cards the member has.

```json List transactions response lines wrap
{
    "data": [
        {
...
<Info>
    ...
    Read the [POST Purchase reward](/api-reference/programs/purchase-reward) endpoint reference documentation to learn more about the exact request and response data.
    
</Info>

Use the POST Purchase reward (`/v2/loyalties/programs/{programId}/members/{memberId}/rewards/purchases`) endpoint to create a store where your loyalty program members can buy rewards.
...
IDEAS:
- Ideate what features you really need
- Check if IDs are input correctly
- Check if CRM connection is correct
- Check member e2e scenario: signup, earning, spending, deactivation
- 
...
<AccordionGroup>
    ...
    <Accordion title="Check audit logs">
