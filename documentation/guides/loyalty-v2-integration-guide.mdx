...
<Steps>
    ...
    Use an existing API key/token or or create a new pair. Go to **Project settings** > **Application keys** to get `X-App-Id` and `X-App-Token`.
...
Your loyalty program should also support situations when a member wants to stop participating in the program, have their member profile deleted altogether, or cases when you need to teomporarily suspend member activity when suspecting program fraud.
...
If you need to have macheanism for deleting member data, use the [DELETE Member](/api-reference/programs/delete-member) endpoint to soft-delete the member and all their associated cards within the program.
