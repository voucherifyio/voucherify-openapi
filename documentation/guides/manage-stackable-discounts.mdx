...
description: "Learn how to stack many redeemables in one API call"
keywords: ['API', 'stackable', 'redeem', 'validate']
---
...
<Note>
  ...
  Read the [Stacking rules article](/orchestrate/stacking-rules) to learn more about the configuration of the stacking rules.
</Note>
...
| REDEMPTIONYou can pass up to 30 redeemables that will be redeemed during the request. The redemption is successful: if all redeemables can be applied i.e. each redeemable is successfully validated before the redemption ([the discount application rule is set to ALL](/orchestrate/stacking-rules#application-and-sorting)).for those redeemables which can be applied, i.e. which succeeded validation. The redeemables which failed validation are inapplicable ([the discount application rule is set to PARTIAL](/orchestrate/stacking-rules#application-and-sorting)). | **POST** `base_URL/v1/redemptions` *for server-side***POST** `base_URL/client/v1/redemptions` *for client-side* |
...
* If all redeemables can be applied, i.e. each redeemable is successfully validated before the redemption ([the discount application rule is set to ALL](/orchestrate/stacking-rules#application-and-sorting)).
* For those redeemables which can be applied, i.e. which succeeded validation. The redeemables which failed validation are inapplicable ([the discount application rule is set to PARTIAL](/orchestrate/stacking-rules#application-and-sorting)).
