...
First, we need to call the [qualification endpoint](/api-reference/qualifications/check-eligibility). It returns a list of loyalty cards that can be used for redeeming a reward specified in the request.
...
The [validation API](/api-reference/validations/validate-stackable-discounts) method should be used to verify whether the customer has enough points for receiving a reward. Here's an endpoint and example request.
