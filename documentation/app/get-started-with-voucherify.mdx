...
If you've created the [FLASH campaign](#set-up-generic-code-campaign), check it out in the Demo shop to learn how the rule engine works in practice.

The *FLASH!* coupon has the following conditions:
...
Now, redeem the coupon successfully:
...
You will see an error message at the bottom of the screen that the coupon can't be applied, because the same customer, Karen Doe, tries to use the coupon for the second time, which doesn't meet the third condition, even though the first two conditions are met.
