...
Voucherify gives you a 60 day trial to play and test the platform for free in a Sandbox project.
...
Alternatively, you can create your first campaign with [the tutorial below](#tutorial-set-up-a-generic-code-campaign).
...
Each project has a **Demo shop**. It shows how the integration between Voucherify and customer touchpoints works in practice. Use **Demo shop** to try out campaigns and check how they work for test customers.
...
<Steps>
  ...
  <Step title="Time frame (skip)">
    Go to **Time frame**, but leave the settings as they are. You don't need to set up when the campaign starts, expires, or when it's active or inactive this time.
  </Step>
...
Once you've created the *FLASH campaign*, check it out in the **Demo shop** to learn how the rule engine works in practice. In this tutorial, you'll check scenarios when validation fails and succeeds depending on different validation rule conditions.
