...
2.  [Conditions](#validation-rules) step – configure validation rules (optional).
3.  [Discount types](#discount-types) – different in particular discount categories.
4.  [Discount value](#discount) – static or dynamic; build formula to create a dynamic discount.
...
| ------------------- | ------ |
| **Order Discount**  | Amount discount: Apply to whole cart<br />Percent discount: Apply to whole cart<br />New price: Apply to whole cart |
| **Products Discount** | Amount discount: Apply the discount to item subtotal<br />Amount discount: Apply the full discount to each item unit<br />Amount discount: Split the discount proportionally to amount<br />Amount discount: Split the discount proportionally to quantity<br />Percent discount: Apply the discount to item subtotal<br />New price: Apply the discount to item subtotal |
| **Rewards** | Free products: Add missing order items to match total discount units count<br />Free products: Always add new units items<br />Free products: Add many order items<br />Upgrade bonus: Replace order items |
| **Free Shipping** | Free products: Add missing order items to match total discount units count |

### Amount discount

This discount defines an amount in any currency that will be subtracted from the whole cart or product, for example, $20 or €10 off. You can set the currency in the [Project Settings](about://support.voucherify.io/article/120-account-settings#general).
...
Product discounts determine how discounts are, or are not, applied to individual [products or collections](https://support.voucherify.io/article/515-products). You can also choose [discounted products and their effects](#choosing-discounted-products-and-effects) and, in some cases, [additional caps or limitations](#selected-products-caps).

These product limits are applied as [validation rules](#validation-rules), which can be reused in the future. In the Conditions step, Use the Configured Options Only option to use only the product limits. To add more validation rules, click With Rules to search for an existing validation rule or create a new one with the plus button. If you are going to reuse an existing rule, Voucherify will clone the rule, so its original configuration will not be overwritten.
...
If you select individual products or collections for a discount, you can further define [additional cap limits](#selected-products-caps).
...
<Accordion title="Percent discount: Apply the discount to item subtotal">
...
<Accordion title="New price discount: Apply the discount to item subtotal">
...
*   [Caps](#selected-products-caps)
*   [Conditions (validation rules)](#validation-rules)
...
This type of discount is based on the items added to the Products section in the Voucherify Dashboard. If you have not uploaded your inventory yet, follow the [Products and Product Collections](https://support.voucherify.io/article/515-products) guide first.
</Note>
...
You can further define [additional cap limits](#selected-products-caps):
...
<Accordion title="Upgrade Bonus: Replace order items 1">
...
<Accordion title="Upgrade Bonus: Replace order items 2">
...
<Accordion title="Upgrade Bonus: Replace order items 3">
...
Click _\+ Add discount_ → _Bundles_, and choose one of the available bundle types.

<Note>
**Read the [detailed guide](https://support.voucherify.io/article/558-product-bundles) about bundles in Voucherify.**
</Note>
...
This adds free products automatically when a customer buys specific items. The available discount types are:
...
Buy 2 notebooks, get 1 pen for free.

### Discount Whole Order (Order discount)

Applies a discount to the entire order when the bundle conditions are met. The available discount types are:
...
Buy 2 laptops and 2 bags, get 10% off the total order.

### Discount Bundle Items by Fixed Amount (Amount product discount)

This applies a fixed amount discount to the products within the bundle. The available discount types are:

*   Amount discount: Apply the full discount to each item unit.
    *   Max. units per order item is set to 1 by default. This means that each bundle discounts one item unit, so if there are two bundles, two item units will be discounted. Otherwise, even one bundle discounts all item units.
    *   Max. units combined for all matched order lines is set to 1 by default.
*   Amount discount: Apply the discount to item subtotal

In Discount, you can copy bundle items from the Bundled items step.

**Example**:

Buy 3 candles, get $10 off the candle bundle total.

### Discount Percentage Off Bundle Items (Percentage product discount)

This applies a percentage discount to all bundled products. The available discount type is Percent discount: Apply the discount to item subtotal.

In Discount, you can copy bundle items from the Bundled items step.

**Example**:

Buy 2 skincare products, get 20% off.

### Set Price for Bundle Items (New unit price discount)

This sets a new price for the entire bundle instead of individual discounts. The available discount type is New price: Apply the discount to item subtotal.

*   Max bundles per order value is set to 1 by default.
*   Max. units combined for all matched order lines is set to 1 by default.
...
Buy a coffee machine and grinder for \$99 (instead of \$129).

### Split Discount Between Bundle Items (Split product discount)

This distributes a single discount value across all bundled products, based on either subtotal or quantity. The available discount types are:
...
Buy 2 shirts and 3 ties, get \$25 off — \$15 applied to shirts (60%), \$10 to ties (40%).
...
[Dynamic discounts](https://support.voucherify.io/article/568-how-to-create-dynamic-discount-value) change the discount value based on defined conditions. All discount values (except for the shipping discount) can be dynamic. Click Build formula to open the dynamic discount formula builder.
...
*   Maximum amount combined for all order lines – in product discounts, it sets the maximum amount of the applied discount to the whole order. This limit is available for:
    *   [Amount discount: Apply the discount to item subtotal](#amount-apply-the-discount-to-item-subtotal)
    *   [Amount discount: Apply the full discount to each item unit](#amount-apply-the-full-discount-to-each-item-unit)
    *   [Percent discount: Apply the discount to item subtotal](#percent-apply-the-discount-to-item-subtotal)
*   [Discount limits for selected products – caps](#selected-products-caps):
...
*   [Amount discount: Apply the full discount to each item unit](#amount-apply-the-full-discount-to-each-item-unit)
*   [Percent discount: Apply the discount to item subtotal](#percent-apply-the-discount-to-item-subtotal)
*   [New price: Apply the discount to item subtotal](#new-price-apply-the-discount-to-item-subtotal)

The limits listed here also work together with [additional discount limits](#additional): Maximum discount amount per order line and Maximum discount amount per order. The discount is always calculated in such a way as to apply the lowest possible discount, whether from the product limit caps, or other limits.

### Maximum discount amount per order line

This limits the amount of the discount for one order line or item subtotal.

**Example –** If set to \$25: The customer orders five discounted product units in three order lines. A maximum of \$25 discount will be applied to each applicable order line.

#### Maximum discount amount per order

This limits the discount amount for the whole cart.

**Example –** If set to \$100: The customer orders five discounted product units in three order lines. The maximum discount for all order lines will reach \$100.
...
**Example –** If set to 3: The customer orders five units of a discounted product. However, only three units will be discounted.
...
**Example –** If set to 5: The customer orders two product units in one order line, and four product units in another order line, making six product units in total. The discount applies to all order lines. Only five units are discounted and the remaining unit has a full price.
...
**Example –** If set to 10, it applies to all customers:
...
**Example –** If set to 5: The customer can buy a maximum of 5 item units with a discount. If they buy a 6th item or more, these items will not be discounted. This limit can be split between orders, so if the customer buys 3 item units in one order, and 3 item units in the second order, 5 item units will be discounted and 1 item unit from the second order will have its full price.
...
**Example –** If set to 5 for an “Adventure brand” collection consisting of T-shirts, beanies, and posters:
...
*   [Upgrade Bonus: Replace order items](#upgrade-bonus-replace-order-items) in the Rewards discount effects,
*   Items covered by a gift card in a [gift card campaign](about://support.voucherify.io/article/47-prepaid-gift-cards-campaign#create-campaign).
...
The application rules can be combined with [the Capped by settings](#selected-products-caps) to limit the maximum discount amount or the number of resources covered by the discount.
...
Optionally, discounts can have conditions defined with [validation rules](https://support.voucherify.io/article/529-validation-rules-campaign-limits), which limit discount application. The validation rules must be met for the discount to be applied.

Use the search field to find and reuse existing validation rules.

To create a new rule, click the plus button to open the [validation rule builder](https://support.voucherify.io/article/148-how-to-build-a-rule) in the Campaign Manager. You can also create validation rules in the [Validation Rules](about://support.voucherify.io/article/16-dashboard-sections#valrules) section in the dashboard for future use.
