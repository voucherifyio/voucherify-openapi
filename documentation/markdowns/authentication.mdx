...
[Suggest Edits](/edit/authentication)
...
<Info>
  In addition to the keys, you will need to choose suitable [API endpoints](/docs/api-endpoints).
</Info>
...
Most likely, you will not have to send your keys manually. See [SDKs](/docs/sdks) that know how to handle them, so no extra code is needed.
...
The other authentication mechanism uses publishable keys. They are meant to be used by the public (web and mobile) clients to access a subset of the API (labeled with `(client-side)` in the [API reference](/docs/api-reference)).

See [voucherify.js](/docs/client-side-api) as an example.
...
Here is an example of a client-side request ([Validate Voucher (client-side)](/reference/validate-voucher)) authorized using client-side API keys.
...
* [Generate OAuth token](/reference/generate-oauth-token)
* [Introspect OAuth token](/reference/introspect-oauth-token)
* [Revoke OAuth token](/reference/revoke-oauth-token)
...
* * [Authorization Overview](#authorization-overview)
  ...
  * [Client-Side Authentication](#client-side-authentication)
    * [Client-Side Request Headers](#client-side-request-headers)
  * [OAuth 2.0](#oauth-20)
