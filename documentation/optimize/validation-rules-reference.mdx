...
<Note>
**Total amount before discounts** checks the cart value before any discounts are calculated, while **Initial amount** checks the cart value before discounts and before other campaign adjustments such as stacked promotions.
</Note>

- **Total amount before discounts**: Required total cart value before discounts are applied.
- **Total amount after discounts**: Required total cart value after discounts are applied. If applying the campaign discount reduces the order total below the configured threshold, the validation fails and the discount is not applied.
